...
death_chance = [0, 0.05, 0.1, 0.15, 0.2, 0.25]
mutation_size = [0.000067, 0.00033]
...
for a in seeds:
    for b in death_chance:
        for c in mutation_size:
            command_str = './symbulation_sgp -SEED '+str(a)+ ' -BASE_DEATH_CHANCE ' +str(b)+ ' -MUTATION_SIZE '+str(c)+' -FILE_NAME _SIZE'+str(c)+'_DEATH'+str(b)
            settings_filename = "Output"+"_SIZE"+str(c)+"_DEATH"+str(b)+"_SEED"+str(a)+".data"
