...
# Organism counts
```{r}
ggplot(subset(counts_df, update %% 10000 == 0),aes(x=update)) + 
...
molten_per_tasks <- subset(molten_tasks, update %% 1500 != 0)
small_per_count <- subset(counts_df, update %% 1500 != 0)
...
```{r}
box_df <- subset(molten_tasks, update == max(update) & host_task==FALSE)
...
ggplot(box_df, aes(x=treatment,y=value)) + 
  ...
  xlab("Symbiont Type") + ylab("Symbiont Task Count") +  scale_color_manual(name="Treatment", values=viridis(4)) + 
